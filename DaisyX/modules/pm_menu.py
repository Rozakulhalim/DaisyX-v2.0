...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add (InlineKeyboardButton(strings['btn_lang'], callback_data='lang_btn'),
                 InlineKeyboardButton(strings['btn_source'], url='https://github.com/Halim77/'))
    buttons.add(InlineKeyboardButton(strings['btn_channel'], url='https://t.me/MRojeck_Lim'),
                InlineKeyboardButton('👥 Support Grup', url='https://t.me/DaisyXUpdates'))
    buttons.add(InlineKeyboardButton("👸🏼 Add Me to your group", url=f'https://telegram.me/Dasha_LimzBot?startgroup=true'))
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(InlineKeyboardButton(
        text=text, url="https://t.me/Dasha_LimzBot?start"))
    await message.reply(strings['help_header'], reply_markup=button)
